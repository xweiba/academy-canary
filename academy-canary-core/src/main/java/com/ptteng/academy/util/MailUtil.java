...
public class MailUtil {
    ...
    // 外部调用发送方法
    // 只提供发送邮箱
    public boolean sendMail(String email) throws ResourceIsNullException {
        return sendMailReal(email, subject, fromName);
    }

    // 发送方法
    private boolean sendMailReal(String email, String subject, String fromName) throws ResourceIsNullException {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if ((Integer) jsonObject.get("statusCode") == 200) {
                    ...
                    throw new ResourceIsNullException("提醒: 邮件发送失败!");
                }
            } else {
                log.debug("请求发送失败 ~");
                throw new ResourceIsNullException("提醒: 邮件发送失败!");
            ...
            log.debug("未捕获异常");
            throw new ResourceIsNullException("提醒: 邮件发送失败! 收件格式错误!");
        }
    }
}
